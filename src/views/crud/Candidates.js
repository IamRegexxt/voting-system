...
import { Table, TableBar, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, TextField } from '@mui/material';
...
const Candidates = () => {
  const style = {
    ...
    const fetchPosition = async () => {
        await axios.get('https://amorganda.tech/public/api/positions', { headers: headers },
        {
            headers: headers
        }).then(({ data }) => {
            setPosition(data)
    ...
    const fetchCandidate = async () => {
        await axios.get('https://amorganda.tech/public/api/candidates', { headers: headers },
        {
            headers: headers
        }).then(({ data }) => {
          setCandidate(data)
    ...
    const fetchUsers = async () => {
      await axios.get('https://amorganda.tech/public/api/users', { headers: headers },
      {
          headers: headers
      }).then(({ data }) => {
        setUsers(data)
    ...
    const updateCandidateInfo = async (e) => {
      ...
      console.log(users_id);
        ...
        await axios.put(`https://amorganda.tech/public/api/candidate/${id}`, formData, { headers: headers }).then(({ data }) => {
          Swal.fire({
      ...
      await axios.delete(`https://amorganda.tech/public/api/candidate/${id}`, { headers: headers }).then(({ data }) => {
      ...
      const createCandidatesModal = () => {
        ...
        const createPosition = async (e) => {
          ...
          await axios.post('https://amorganda.tech/public/api/candidate', formData, { headers: headers }).then(({ data }) => {
        ...
        <TableHead>
          <TableRow>
            ...
            <TableCell style={{fontSize: '22px'}}><center><strong>Action
              <Button variant="contained" onClick={createCandidatesModal}>Create</Button>
              </strong></center>
            </TableCell>
          </TableRow>
        </TableHead>
                ...
                return (
                  <TableRow key={key} style={{textAlignLast: 'center'}} >
                    ...
                    <TableCell style={{fontSize: '20px'}}>
                      <Button color="secondary" onClick={() => readCandidates(column.id,column.last_name,column.first_name,position.position_name)}>
                        Read
                      </Button>
                      <Button variant="contained" color="success" onClick={() => updateCandidates(column.id,column.last_name,column.first_name,column.users_id,column.position_id)}>
                        Update
                      </Button>
                      <Button variant="outlined" color="error" onClick={() => deleteCandidate(column.id)}>
                        Delete
                      </Button>
                    </TableCell>
                  </TableRow>
              ...
              <TableHead>
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell style={{fontSize: '20px'}} id="users_id" value={users_id}
              ...
              <TableHead>
                ...
                <TableBody>
                  <TableRow>
                    <TableCell style={{fontSize: '20px'}}>User:</TableCell>
                    <TableCell style={{fontSize: '20px'}}>
                      <TextField id="users_id" value={users_id} label="User"
                      onChange={(event) => setUserId(event.target.value)}/>
                    </TableCell>
                  </TableRow>
                  <TableRow>
